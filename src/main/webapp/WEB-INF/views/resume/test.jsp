                  ...
                  <br><br><br><br><br>
                   ...
                   <c:forEach items="${ resumeList }" var="resume2">
						                   ...
						                   <td>
						                	 	<c:forEach items="${ resume2.resume_school_classification }" var="schoolc">
													<select name="resume_school_classification" class="form-control">
							                           <option value="${ schoolc }" ${ schoolc eq '고등학교' ? 'selected="selected"' : '' } >고등학교</option>
							                           <option value="${ schoolc }" ${ schoolc eq '2~3년제 대학' ? 'selected="selected"' : '' }>2~3년제 대학</option>
							                           <option value="${ schoolc }" ${ schoolc eq '4년제 대학' ? 'selected="selected"' : '' }>4년제 대학</option>
							                           <option value="${ schoolc }" ${ schoolc eq '대학원' ? 'selected="selected"' : '' }>대학원</option>
							                        </select>
												</c:forEach>
										  	</td>
						                	 <td>
						                	 	<c:forEach items="${ resume2.resume_entrance_date }" var="ent">
						                    		<input type="text" size="15" name="resume_entrance_date" value="${ ent }"><br>              	
										  		</c:forEach>
										  	</td>
										  	<td>
						                	 	<c:forEach items="${ resume2.resume_graduate_date }" var="gra">
						                    		<input type="text" size="15" name="resume_graduate_date" value="${ gra }"><br>              	
										  		</c:forEach>
										  	</td>
										  	<td>
						                	 	<c:forEach items="${ resume2.resume_school }" var="sch">
						                    		<input type="text" size="15" name="resume_school" value="${ sch }"><br>              	
										  		</c:forEach>
										  	</td>
										  	<td>
						                	 	<c:forEach items="${ resume2.resume_major }" var="maj">
						                    		<input type="text" size="15" name="resume_major" value="${ maj }"><br>              	
										  		</c:forEach>
										  	</td>
										  	<td>
						                	 	<c:forEach items="${ resume2.resume_graduate_yn }" var="gray">
													<select name="resume_graduate_yn" style="width:100px;" class="form-control">
						                                <option value="졸업" ${ gray eq '졸업' ? 'selected="selected"' : '' }>졸업</option>
						                                <option value="졸업예정" ${ gray eq '졸업예정' ? 'selected="selected"' : '' }>졸업예정</option>
						                                <option value="재학중" ${ gray eq '재학중' ? 'selected="selected"' : '' }>재학중</option>
						                                <option value="중퇴" ${ gray eq '중퇴' ? 'selected="selected"' : '' }>중퇴</option>
						                                <option value="휴학" ${ gray eq '휴학' ? 'selected="selected"' : '' }>휴학</option>
						                         	 </select><br>
											</c:forEach>
										  	</td>
										 </tr>
							...
							<%-- <div id="school">		
							 <c:forEach items="${ resumeList }" var="resume2">
								<c:if test="${ resume.resume_no eq resume2.resume_no }">
 								<div class="dele[resume2]">
									<div style="float:left;">
										<c:forEach items="${ resume2.resume_school_classification }" var="schoolc">
											<select name="resume_school_classification" class="form-control">
					                           <option value="고등학교" ${ schoolc eq '고등학교' ? 'selected="selected"' : '' } >고등학교</option>
					                           <option value="2~3년제 대학" ${ schoolc eq '2~3년제 대학' ? 'selected="selected"' : '' }>2~3년제 대학</option>
					                           <option value="4년제 대학" ${ schoolc eq '4년제 대학' ? 'selected="selected"' : '' }>4년제 대학</option>
					                           <option value="대학원" ${ schoolc eq '대학원' ? 'selected="selected"' : '' }>대학원</option>
					                        </select><br>
										</c:forEach>
									</div>
									<div style="float:left;">
										<c:forEach items="${ resume2.resume_entrance_date }" var="ent">
											<input type="text" size="20" name="resume_entrance_date" value="${ ent }"><br>
										</c:forEach>
									</div>
									<div style="float:left;">
										<c:forEach items="${ resume2.resume_graduate_date }" var="gra">
											<input type="text" size="20" name="resume_graduate_date" value="${ gra }"><br>
										</c:forEach>
									</div>
									<div style="float:left;">
										<c:forEach items="${ resume2.resume_school }" var="school">
											<input type="text" size="20" name="resume_school" value="${ school }"><br>
										</c:forEach>
									</div>
									<div style="float:left;">
										<c:forEach items="${ resume2.resume_major }" var="schoolm">
											<input type="text" size="20" name="resume_major" value="${ schoolm }"><br>
										</c:forEach>
									</div>
									<div style="float:left;">
										<c:forEach items="${ resume2.resume_graduate_yn }" var="gray">
							...
							<br><br><br><br><br>
                 <br> <span id="addSchool" onclick="school();">+ 학력추가</span>
               </div>
